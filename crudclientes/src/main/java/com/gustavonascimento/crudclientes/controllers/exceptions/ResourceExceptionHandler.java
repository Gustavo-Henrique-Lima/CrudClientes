package com.gustavonascimento.crudclientes.controllers.exceptions;
...
import com.gustavonascimento.crudclientes.services.exceptions.DataBaseException;
import com.gustavonascimento.crudclientes.services.exceptions.ResourceNotFoundException;
...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<ValidationError> validation(ConstraintViolationException e, HttpServletRequest request) {
		...
		List<FieldMessage> fieldMessages = e.getConstraintViolations().stream()
				.map(violation -> new FieldMessage(violation.getPropertyPath().toString(), violation.getMessage()))
